...
function _sp2d_tiled_load_tile!(shared_xi, shared_ui, shared_xj, shared_uj, x_mat, u_mat, ti, tj, i0, j0, ni, nj, N_points, lid, workgroup_size)
    if ni > 0 && nj > 0
        k = lid
        while k <= ni
            gi = i0 + k - 1
    ...
    if bid <= n_tile_blocks
        ...
        if ni > 0 && nj > 0
            ...
            while p <= n_pairs
                ...
                bin = _gpu_digitize_linear(dist, dist_first, dist_last, dist_inv_step, dist_offset, dist_step, N_bins)
                if 1 <= bin < N_bins
                    dU = U2 - U1
                    r̂ = dX / dist
                    ...
                    _gpu_accumulate_single_pass_2d_pair_global_linear_val_cols!(
                        output_sums, output_counts, val_first, val_last, val_inv_step, val_offset, val_step,
            ...
            while p <= n_pairs
                ...
                if 1 <= bin < N_bins
                    ...
                    _gpu_accumulate_single_pass_2d_pair_global_inflinear_val!(
                        output_sums, output_counts, bin, du_L, du_T, du_L2, du_T2, N_val_edges,
            ...
            while p <= n_pairs
                ...
                if 1 <= bin < N_bins
                    ...
                    _gpu_accumulate_single_pass_2d_pair_global_inflinear_val_cols!(
                        output_sums, output_counts, val_first, val_last, val_inv_step, val_offset, val_step, inner_last,
            ...
            while p <= n_pairs
                ...
                if 1 <= bin < N_bins
                    ...
                    _gpu_accumulate_single_pass_2d_pair_global_log_val!(
                        output_sums, output_counts, val_first, val_last, val_inv_step, val_offset, val_step, bin,
