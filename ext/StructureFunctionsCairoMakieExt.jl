...
"""
function SFSA.compare_spectral_analysis(
    input_data,
    results;
    peaks = nothing,
    u_idx = 1,
    ...
    # 1. Plot Input Field
    ax_field = CairoMakie.Axis(
        fig[1, 1];
        title = "Input Field (u[$u_idx])",
        xlabel = D == 1 ? "x" : "x",
        ylabel = D == 1 ? "u" : "y",
    )
    u_plot = u_vecs[u_idx]
    ...
    for (i, (label, (coeffs, k_ranges))) in enumerate(results)
        ...
        if D == 1
            ...
            hm = CairoMakie.heatmap!(
                ...
                k_ranges[1],
                k_ranges[2],
                abs.(Base.selectdim(coeffs, 3, u_idx)),
            )
            if peaks !== nothing
                p_kx = [p[1] for p in peaks]
                p_ky = [p[2] for p in peaks]
