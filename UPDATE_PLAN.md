...
## Block I
1. Commit I1: Support for real data. We should have extension support for JLD2, CSV (and similar txt files), NetCDF, Zarr, HDF5, etc.  This is a large undertaking -- for example a lot of these files are spread over multiple datasets so we should have additional support for out of core computation with DiskArrays or whatever the Julia SOTA is these days (it is very common to want to calculate the structure functions or spectra of data that absolutely do not fit in memory and are split over many files). This also will necessitate NaN handling, turning ND data to lists of points, etc... a lot of work to do... There may be some common utilities these can share but a lot of the code i imagine will be pretty separate. Maybe we should have a folder with alll the real data extensions and hooks.
2. Commit I2: We need to create and exercise tests for all these real-data extension methods.

## Workflow Audit Findings (2026-03-17)
- `CI.yml` and `ci.yml` are redundant. Merge into single `ci.yml`.
- `CompatHelper.yml` is obsolete; remove in favor of `dependabot.yml`.
- `UpDeps.yml` is legacy and targets Julia 1.9.3; remove and rely on Dependabot for action/dependency updates.
- Standardize all action versions: `checkout@v4`, `setup-julia@v2.6.0`, `cache@v2`, `codecov@v5.1.2`, `paths-filter@v3.0.2`.

## API Re-alignment: Operators vs. Result Containers
- **Problem**: `StructureFunction` is currently a minimal container that is not used by `calculate_structure_function`. The "Structure Function Types" (operators) use names that overlap with the container concept.
- **Solution**:
    - Re-engineer `StructureFunction` as the primary **Result Container**.
    - Update it to be generic (support tuples, SVectors, etc.) and hold metadata (order, distance bins, operator type).
    - Refactor `calculate_structure_function` to return a `StructureFunction` object instead of a raw vector.
    - Clarify naming: Subtypes of `AbstractStructureFunctionType` are **Operators**.


## Block J: final cleanup and release prep

1. Commit J1: Source Cleanup.
   - Move archive candidates out of `src/`. Delete unused archive/prototype code and commented-out blocks.
2. Commit J2: GitHub Workflow Modernization & Deduplication.
   - Delete `CI.yml`, `CompatHelper.yml`.
   - Consolidate into single `ci.yml` (merging logic from `CI` and `ci`).
   - Standardize all action versions: `checkout@v4`, `setup-julia@v2.6.0`, `cache@v2`, `codecov@v5.1.2`, `paths-filter@v3.0.2`.
   - Julia 1.12-only matrix (Manual lock).
3. Commit J3: Operator Naming Refinement.
   - Rename all subtypes of `AbstractStructureFunctionType` to use the `Type` suffix (e.g., `ProjectedStructureFunctionType`, `FullVectorStructureFunctionType`, `Longitudinal2ndOrderStructureFunctionType`).
   - This ensures consistency with the abstract base `AbstractStructureFunctionType`.
   - We need shorthand aliases both for the types (e.g. `L2SFType = Longitudinal2ndOrderStructureFunctionType`) and for singleton objects (e.g. `L2SF = Longitudinal2ndOrderStructureFunctionType()`) where `Longitudinal2ndOrderStructureFunctionType` and `Longitudinal2ndOrderStructureFunction` are the longhand aliases of the native types. same for the other aliases.
   - For calculate_structure_function, we should move the operator type to the beginning of the argument list... additionally, the objects themselves can probably make functor calls to calculate structure functions so we should permit that, and for now, route that  to the overall calculate_structure_fucntions method.
4. Commit J4: Re-engineer `StructureFunction` Result Container.
   - Update `StructureFunction` in `src/StructureFunctions.jl` to be a robust, parametric container:
     `struct StructureFunction{FT, OT<:AbstractStructureFunctionType, BT<:AbstractVector, VT<:AbstractVector} <: AbstractStructureFunction`
   - Fields: `order::Int`, `operator::OT`, `distance::BT`, `values::VT`.
   - This ensures it can hold any valid container type and carry metadata about the physical operator used.
   - calculate_structure_functions should return StructureFunction objects instead of raw data/tuples. Also when we have sums and counts that should be another subtype of AbstractStructureFunction.
   - Ensure our framework has the capacity to store bins or points, etc as the coordinate. Then if a StructureFunction is created with bin edges, we can have methods to convert it to a StructureFunction with bin midpoints, etc. 
5. Commit J5: Refactor Calculation Entry Points.
   - Update `calculate_structure_function` (all variants) to return a `StructureFunction` result object instead of raw data/tuples.
6. Commit J6: Type Stability & Boolean Decoupling.
   - Fix `return_sums_and_counts` type instability using `Val(bool)` dispatch.
   - Add convenience methods: `calculate_structure_function_sums_and_counts` which internally calls the `Val(true)` path.
   - Ensure `verbose` and `show_progress` have zero effect on return types. Additionally, i'm not fully sure verbose and show_progress are being respected right now. when we have verbose we should print information, and when we have show_progress we should show a progress bar. ensure this is actually true. I'm not sure if booleans and print statemetn have any overhead, if they do we should probably also move them to a val system with default false, or just put verbose and show_progress in an options type with nice constructors that internally creates the vals itself.
8. Commit J8: Performance Audit (`@inbounds`).
   - Rigorous sweep of hot loops in `Calculations.jl` and `HelperFunctions.jl`.
9. Commit J9: Final Test Suite Harmonization.
   - Ensure `runtests.jl` exercises all extensions and the new unified container API.
   - Implement `test/test_type_stability.jl` for exhaustive package-wide verification.
10. Commit J10: Comprehensive README Overhaul (Manual).
    - Sections: Architecture (Operator/Container split), Performance, Scaling, API Reference, Extension Guides.
    - Path: `docs/README.md` for internal documentation details.
    - This is probably the most work of all the commits, as it entails documenting the entire package. You will need to make a plan for this to ensure nothing is overlooked.
    - Be sure to link to sources for refernce, any listed so far, https://dhruvbalwada.github.io/assets/documents/Balwada_et_al2016b_supp_material.pdf, https://doi.org/10.1073/pnas.0600482103, [wikipedia](https://en.wikipedia.org/wiki/Turbulence#Kolmogorov's_theory_of_1941), etc
11. Commit J11: Final Release Prep (v0.3.0).
    - Add migration notes and changelog draft.
    - Version bump and release notes.

## Future Milestones
1. **Block K (v1.0)**:
   - Bump to v1.0.
   - Final registration via `JuliaRegistrator`.
...
- 2026-03-17:
  - **Commit fa33a3ad756b14e74543a57b7fdc207d523a5063 (Block J)**:
    - **Import Consolidation**: Unified all submodule imports into concise `using StructureFunctions: ...` statements across core modules, tests, and extensions.
    - **Extension Architecture**: Renamed all extensions to `StructureFunctions<Name>Ext` and updated filenames.
    - **API Restoration**: Restored `magnitude_δu_longitudinal`, `SF_TYPE_MAP`, and `SpectralAnalysis` plotting stubs.
    - **Technical Debt**: Cleaned up redundant `as` aliases and established strict `using Module: Module` architecture.
    - **KA Workaround**: Documented the current necessity for explicit `@index` and `@atomic` imports from `KernelAbstractions` (Reference: https://github.com/JuliaGPU/KernelAbstractions.jl/issues/542#issuecomment-3986160961).
    - **Verification**: Fully validated v0.3.0 with passing tests, JET audit, and 100% Aqua compliance.
  - **Block I (Real Data Support)**:
    - **Scientific Formats**: Implemented extension-based support for `JLD2`, `NetCDF`, `Zarr`, `HDF5`, and `CSV`.
    - **Coordinate Expansion**: Automated mapping of 1D coordinate vectors to N-dimensional grids.
    - **Parity Verification**: Confirmed numerical agreement between file-based and in-memory calculations ($rtol < 1e-12$).
  ...
  - **Commit J3 (Naming & Signature Refinement)**:
    ...
    - Implemented identity functors for all SF types and operators.
    - Updated all test suites (`core`, `parallel`, `gpu`, `jet`, `performance`, `extensions`) to match new signatures.
    - Verified 100% functional parity and type stability.
