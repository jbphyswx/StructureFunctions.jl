...
Test.@testset "Inputs: Exhaustive Input Matrix Tests" begin
    ...
    N_side = 10
    N_total = N_side^2
    xs_std, ys_std, _ = SyntheticData.generate_nonuniform_domain(N_side; R_mask=0.0, lat_range=(-5.0, 5.0), lon_range=(0.0, 10.0))
    u_std = SyntheticData.generate_spectral_field(xs_std, ys_std; peaks=peaks, noise_level=0.0)
    # DirectSum is our ground truth
    r_ref = SFSA.calculate_spectrum(SFSA.DirectSumBackend(), (xs_std, ys_std), (real(u_std),), ms; domain_size=(L,L))
    c_ref, k_ref = r_ref
    ...
    Test.@testset "Coordinate Types" begin
        ...
        c, k = SFSA.calculate_spectrum(SFSA.DirectSumBackend(), (xs_s, ys_s), (real(u_std),), ms; domain_size=(L,L))
        Test.@test c ≈ c_ref
        
        # Test 2: Views and SubArrays
        xs_v = view(xs_std, 1:N_total)
        ys_v = view(ys_std, 1:N_total)
        c, k = SFSA.calculate_spectrum(SFSA.DirectSumBackend(), (xs_v, ys_v), (real(u_std),), ms; domain_size=(L,L))
        Test.@test c ≈ c_ref
    end

    Test.@testset "Field Types" begin
        # Test 1: Multiple fields (NU > 1)
        u2 = SyntheticData.generate_spectral_field(xs_std, ys_std; peaks=[(dk*3, 0.0, 1.0)], noise_level=0.0)
        c, k = SFSA.calculate_spectrum(SFSA.DirectSumBackend(), (xs_std, ys_std), (real(u_std), real(u2)), ms; domain_size=(L,L))
        Test.@test size(c, 3) == 2
        Test.@test c[:,:,1] ≈ c_ref[:,:,1]

        # exp(im * k * x) with forward transform e^(-im * k * x) peaks at +k
        c, k = SFSA.calculate_spectrum(SFSA.DirectSumBackend(), (xs_std, ys_std), (u_std,), ms; domain_size=(L,L))
    ...
    Test.@testset "Backend Robustness with Views" begin
        ...
        c_n, k_n = SFSA.calculate_spectrum(SFSA.FINUFFTBackend(), (xs_v, ys_v), (u_v,), ms; domain_size=(L,L), eps=1e-12)
        ...
        uv = real.(SyntheticData.generate_spectral_field(xv, yv; peaks=peaks, noise_level=0.0))
        
        c_f, k_f = SFSA.calculate_spectrum(SFSA.FFTBackend(), (view(xv,:), view(yv,:)), (view(uv,:),), (N_u, N_u); domain_size=(L,L))
