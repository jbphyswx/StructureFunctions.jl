...
Test.@testset "Spectral Recovery: Tiered Verification" begin
    ...
    Test.@testset "Uniform Grid Bit-Parity" begin
        ...
        L = 10.0
        r_direct = SFSA.calculate_spectrum(
        ...
        r_nufft = SFSA.calculate_spectrum(
            SFSA.FINUFFTBackend(),
            (xv, yv),
            (real(u),),
            ms;
        ...
        fig = SFSA.compare_spectral_analysis(((xv, yv), (real(u),)),
            ...
            title = "Uniform Grid Parity Comparison (u=real, noise=0.0)")
        CM.save(joinpath(PLOT_DIR, "verify_spectral_parity_uniform_grid.png"), fig)
    end
    ...
    Test.@testset "Scattered Grid Parity (Noisy)" begin
        ...
        xs, ys, mask = SyntheticData.generate_nonuniform_domain(
            N_side_gen;
            R_mask = 5.0,
            lat_range = (-5.0, 5.0),
            lon_range = (0.0, 10.0),
        ...
        CM.save(joinpath(PLOT_DIR, "verify_spectral_parity_scattered_grid.png"), fig)
    end

    Test.@testset "Sampling Validation (High-res FFT vs Scattered NUFFT)" begin
        # Generate perfect high-res signal
        N_high = 128
        xh, yh, uh = SyntheticData.generate_highres_uniform(
            N_high;
            lat_range = (-5.0, 5.0),
            lon_range = (0.0, 10.0),
            peaks = peaks,
        ...
        CM.save(joinpath(PLOT_DIR, "verify_spectral_sampling_validation.png"), fig)
    end
end
