...
function calculate_structure_function(
    ...
    output = MVector{N3, FT2}(zeros(FT2, N3)) # allocation checking
    counts = MVector{N3, Int64}(zeros(Int64,N3))
    ...
    @threads for i in iter_inds # is this the fast order?
...
function calculate_structure_function_i(
    ...
    output = MVector{N3-1, FT2}(zeros(FT2, N3-1)) # allocation checking
    counts = MVector{N3-1, Int64}(zeros(Int64, N3-1))
    ...
    for j in iter_inds
        if i != j # this has δx and δu = 0, so we skip it
            ...
            @inbounds distance::Union{FT1,FT2} = distance_metric(x_vecs[i], x_vecs[j]) # this is the slow part (has an allocation too...)
            ...
            @inbounds output[bin] += structure_function_type.method(u_vecs[j] - u_vecs[i], HelperFunctions.r̂(x_vecs[i],x_vecs[j])) # the work happens here, with allocations.... this is the slow part
...
function minmax_i(
    ...
    # @inbounds X1 = x_vecs[i] # this probably has unnecssary allocations
    for j in eachindex(x_vecs)
        if i != j
            # @inbounds X2 = x_vecs[j]
            # update the min and max distances
            # distance = distance_metric(X1, X2) # this is the slow part
            @inbounds distance = distance_metric(x_vecs[i], x_vecs[j]) # this is the slow part
